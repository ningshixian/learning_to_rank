...
2020.12.08
test:  0.9716480106115271
斯皮尔曼相关性系数: SpearmanrResult(correlation=0.41943375971241187, pvalue=1.1777273776781963e-255)
皮尔森相关性系数: (0.5709523450569733, 0.0)

2021.02.23
train: 0.9960792601890783
test : 0.990606169821969
斯皮尔曼相关性系数: SpearmanrResult(correlation=0.5335438627680218, pvalue=2.569144668360068e-224)
皮尔森相关性系数: (0.6947760913954865, 0.0)
"""
...
STATIC_PARAMS = {
    ...
    "num_threads": 6,  # 线程数=2，可以限制模型训练时CPU的占用率！！！http://ponder.work/2020/01/25/lightgbm-hang-in-multi-thread/
    # "device":"gpu"
...
def train(t_number, params):
    ...
    # gbm.save_model(setting.model_bucketA + '{}.mod'.format(t_number), num_iteration=gbm.best_iteration)  # 会保存最佳迭代结果:
    with open(setting.model_bucketA + setting.gbdt_model.format(t_number), "wb") as f:
        # f.seek(0)
        pkl.dump(gbm, f)

    # 保存当前trial的最佳迭代次数
    with codecs.open(setting.model_bucketA + setting.best_iteration.format(t_number), "w", "utf-8") as f:
...
def test_data_excel(model_path, test_path):
    ...
    df = pd.DataFrame(res)
    df.to_excel(setting.data_bucketA + setting.ltr_output_excel, index=False)
...
def plot_print_feature_importance(model_path):
    ...
    # plt.savefig(setting.model_bucketA + "feat_importance.png")
    ...
    # 导出特征重要性
    f = open(setting.model_bucketA + setting.feature_txt, "w+")
...
def train_lr(model_path):
    ...
    with open(setting.model_bucketA + 'LR.mod', "wb") as f:
        pkl.dump(lm, f)
        print("LR模型保存")
...
if __name__ == "__main__":
    ...
    cate_cols = []
    # cate_cols = joblib.load(setting.data_bucketA + setting.cat_feat_file)
    # cate_cols = list(range(cate_cols, 0))
    # cate_cols = [feature_name_all[idx] for idx in cate_cols]
    # print("{}是类别特征".format(cate_cols))

    raw_data_path = setting.data_bucketA + setting.ltr_train
    data_feats = setting.data_bucketA + setting.lxh_train
    data_group = setting.data_bucketA + setting.lxh_train_group

    # 获取最佳模型路径及最佳迭代次数
    best_model_path, best_iteration = None, None
    if os.path.exists(setting.model_bucketA + setting.best_trial):
        with codecs.open(setting.model_bucketA + setting.best_trial, "r", "utf-8") as f:
            best_trial_number = int(f.read().strip('\n'))
        best_model_path = setting.model_bucketA + "{}.mod".format(best_trial_number)
        with codecs.open(
            setting.model_bucketA + "{}.best_iteration.txt".format(best_trial_number),
            "r",
            "utf-8",
        ) as f:
    ...
    # 加载验证数据
    x_dev, y_dev, q_dev = load_data(setting.data_bucketA + setting.lxh_valid, setting.data_bucketA + setting.lxh_valid_group)
    ...
    # 加载测试数据
    raw_test_path = setting.data_bucketA + setting.ltr_test
    test_group = setting.data_bucketA + setting.lxh_test_group
    ...
    if sys.argv[1] == "-train":
        ...
        # 删除非最佳模型文件
        ls = os.listdir(setting.model_bucketA)
        for name in ls:
            if not name.split(".")[0] == str(study.best_trial.number) and (
                name.endswith(".best_iteration.txt") or name.endswith(".mod")
            ):
                os.remove(setting.model_bucketA + name)

    elif sys.argv[1] == '-plottree':
        #可视化树模型
        save_plot_path = setting.data_bucketA + "plottree"  # 格式如ranklib中的数据格式
        plot_tree(best_model_path, 2, save_plot_path)
        ...
        with codecs.open(
            setting.model_bucketA + setting.best_iteration.format(model_data["trial"]), "r", "utf-8"
        ) as f:
            model_data["iter"] = int(f.read())
        with open(setting.model_bucketA + setting.gbdt_model.format(model_data["trial"]), "rb") as fin:
            model_data["model64"] = base64.b64encode(fin.read()).decode("utf-8")
        # 特征相关的
        model_data["feature_name_all"] = list(feature_name_all)
        if "ctr" in feature_name_all:
            with open(setting.data_bucketA + setting.ctr_file, "rb") as fin:
                model_data["ctr"] = base64.b64encode(fin.read()).decode("utf-8")
        if "tfidf_cos_sim" in feature_name_all:
            with open(setting.data_bucketA + setting.tfidf_vec_file, "rb") as fin:
                model_data["tfidf_cos_sim"] = base64.b64encode(fin.read()).decode("utf-8")
        if "time_diff" in feature_name_all:
            with open(setting.data_bucketA + setting.time_feats_file, "rb") as fin:
        ...
        predict_data_path = setting.data_bucketA + setting.ltr_test  # 格式如ranklib中的数据格式
        ...
        # ndcg
        test_path = setting.data_bucketA + setting.ltr_test  # 评估测试数据的平均ndcg
        test_data_ndcg(best_model_path, test_path)

    elif sys.argv[1] == "-test":  # 可视化结果
        test_path = setting.data_bucketA + setting.ltr_test
        ...
        raw_data = setting.data_bucketA + setting.ltr_test
        with open(raw_data, "r", encoding="utf-8") as testfile:
            test_X, test_y, test_qids, comments = read_dataset(testfile)
